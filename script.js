...
function saveIPKToDatabase(nim, ipk) {
    const xhr = new XMLHttpRequest();
    xhr.open("POST", "save-ipk.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
            alert("IPK berhasil disimpan ke database");
        }
    };
    xhr.send("nim=" + nim + "&ipk=" + ipk);
}
...
function hitungIPK() {
    ...
    saveIPKToDatabase(mahasiswaTarget.nim, ipk);
}
